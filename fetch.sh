...
fetch() {
  ...
  if [[ ! -f "$archive" ]]; then
    ...
    if ! run curl -L --fail --silent --show-error -o "$archive_tmp" "$url"; then
      rm -f "$archive_tmp"
      return 1
    fi
  ...
  dest_tmp=$(mktemp -d "$dest.XXXXXX")
  if ! run tar -xf "$archive" -C "$dest_tmp" --strip-components=1; then
    rm -rf "$dest_tmp"
    rm -f "$archive"
    return 1
  fi
